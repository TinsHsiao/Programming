...
{
	// Open Image File
	ifstream ifile("a.bmp", ios::binary);
	...
	for(int i=0; i<height*2; i++)
		for(int j=0; j<width*2; j++)
			for(int k=0; k<bits_px/8; k++) {
				...
				rgb2[i][j][k] = rgb[i/2][j/2][k];
				
			}
			
				
	// 1 1 > 1 5,  1 5 > 5 5,  5 5 > 5 1
	...
	int file_size = width*2 * height*2 * bits_px/8 + 54;
	...
	// Write New Image
	ofstream of1("2_times_of_width_height.bmp", ios::binary);	
